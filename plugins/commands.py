    ...
    try:
        ...
        if True: 
            try:
                ...
                if fsub_channels:
                    btn += await is_subscribed(client, message.from_user.id, fsub_channels, chat_id=grp_id)
        ...
        if data.startswith("allfiles"):
            try:
                ...
                for file in files:
                    ...
                    MOVIEBOT_CAPTION = settings.get('caption', CUSTOM_FILE_CAPTION)
                    if MOVIEBOT_CAPTION:
                        try:
                            f_caption = MOVIEBOT_CAPTION.format(file_name='' if title is None else title, file_size='' if size is None else size, file_caption='' if f_caption is None else f_caption)
                        except Exception as e:
                            logger.exception(e)
                            f_caption = f_caption
        ...
        if not files_:
            ...
            try:
                ...
                if COVERX:
                    ...
                    cover = details[0].cover if details else None
                btn = await stream_buttons(message.from_user.id, file_id)
                msg = await client.send_cached_media(
                    chat_id=message.from_user.id,
                    cover=cover,
                ...
                if MOVIEBOT_CAPTION:
                    try:
                        f_caption=MOVIEBOT_CAPTION.format(file_name= '' if title is None else title, file_size='' if size is None else size, file_caption='')
                    except:
                        return
...
async def log_file(bot, message):
    """Send log file"""
    try:
        await message.reply_document('Moviebot_logs.txt', caption="📑 **ʟᴏɢꜱ**")
    except Exception as e:
...
async def set_tutorial(client, message: Message):
    ...
    try:
        ...
        return await message.reply_text(
            ...
            f"<code>/{message.command[0]} https://t.me/newlinemovies</code>"
        )
...
async def reset_all_settings(client, message):
    try:
        reset_count = await db.moviebot_reset_settings()
        await message.reply_text(
