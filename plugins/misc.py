...
import logging
from database.ia_filterdb import moviebot_get_movies, moviebot_get_series
...
async def moviebot_list_movies(client, message):
    try:
        movies = await moviebot_get_movies()
        if not movies:
            await message.reply("No movies found in database.")
            return
        out = "Recent Movies In DB Are:\n\n"
        for movie in movies:
            out += f"• {movie}\n"
        await message.reply(out)
    except Exception as e:
        logger.error(f"Error in moviebot_list_movies: {e}")

@Client.on_message(filters.private & filters.command("series"))
async def moviebot_list_series(client, message):
    try:
        series_data = await moviebot_get_series()
        if not series_data:
            await message.reply("No series found in database.")
            return
        out = "Recent Series In DB Are:\n\n"
        for title, seasons in series_data.items():
            seasons_str = ", ".join(map(str, seasons))
            out += f"• {title} (Seasons: {seasons_str})\n"
        await message.reply(out)
    except Exception as e:
        logger.error(f"Error in moviebot_list_series: {e}")
