...
def get_filter_buttons(key):
    """Helper to generate standard filter buttons (Quality, Language, Season, etc.)"""
    return [
        [
...
async def next_page(bot, query):
    ...
    if settings.get('button'):
        ...
        btn = get_filter_buttons(key) + btn
    else:
        btn = get_filter_buttons(key)
    if ULTRA_FAST_MODE:
        if 0 < offset <= 10:
            off_set = 0
        elif offset == 0:
    ...
    if not settings["button"]:
        ...
        moviebot_title = clean_search_text(search)
        cap = None
        try:
            if settings['imdb']:
                cap = await get_cap(settings, remaining_seconds, files, query, total, moviebot_title, offset)
                ...
                cap = await get_cap(settings, remaining_seconds, files, query, total, moviebot_title, offset+1)
                await query.message.edit_text(text=cap, reply_markup=InlineKeyboardMarkup(btn), disable_web_page_preview=True, parse_mode=enums.ParseMode.HTML)
        except MessageNotModified:
            pass
        except Exception as e:
...
async def filter_qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    if not settings["button"]:
        ...
        remaining_seconds = "{:.2f}".format(time_difference.total_seconds())
        moviebot_title = clean_search_text(search)
        cap = await get_cap(settings, remaining_seconds, files, query, total_results, moviebot_title, offset=1)
        try:
            await query.message.edit_text(text=cap, reply_markup=InlineKeyboardMarkup(btn), disable_web_page_preview=True, parse_mode=enums.ParseMode.HTML)
        except MessageNotModified:
            pass
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn: list[list[InlineKeyboardButton]] = []
    if settings.get("button"):
        buttons = [[InlineKeyboardButton(text=f"🔗 {get_size(f.file_size)} ≽ " + clean_filename(f.file_name), callback_data=f"file#{f.file_id}")] for f in files]
        btn = get_filter_buttons(key) + buttons
    else:
        btn = get_filter_buttons(key)
    if n_offset != "":
        try:
    ...
    if not settings.get("button"):
        ...
        moviebot_title = clean_search_text(search_final)
        cap = await get_cap(settings, remaining_seconds, files, query, total_results, moviebot_title, offset=1)
        try:
            await query.message.edit_text(
                text=cap,
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
    ...
    elif MoviebotData.startswith("generate_stream_link"):
        _, file_id = MoviebotData.split(":")
        try:
            user_id = query.from_user.id
            username = query.from_user.mention
            log_msg = await client.send_cached_media(chat_id=BIN_CHANNEL, file_id=file_id,)
            fileName = {quote_plus(get_name(log_msg))}
            moviebot_stream = f"{URL}watch/{str(log_msg.id)}/{quote_plus(get_name(log_msg))}?hash={get_hash(log_msg)}"
            moviebot_download = f"{URL}{str(log_msg.id)}/{quote_plus(get_name(log_msg))}?hash={get_hash(log_msg)}"
            ...
            await log_msg.reply_text(
                ...
                quote=True,
                disable_web_page_preview=True,
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🚀 ꜰᴀꜱᴛ ᴅᴏᴡɴʟᴏᴀᴅ 🚀", url=moviebot_download),  # we download Link
                                                    InlineKeyboardButton('🖥️ ᴡᴀᴛᴄʜ ᴏɴʟɪɴᴇ 🖥️', url=moviebot_stream)]])  # web stream Link
            )
            moviebot_msg = await query.edit_message_reply_markup(
                reply_markup=InlineKeyboardMarkup([
                    [
                        InlineKeyboardButton("🚀 ꜰᴀꜱᴛ ᴅᴏᴡɴʟᴏᴀᴅ 🚀", url=moviebot_download),
                        InlineKeyboardButton('🖥️ ᴡᴀᴛᴄʜ ᴏɴʟɪɴᴇ 🖥️', url=moviebot_stream)
                    ],
                    [
        ...
        moviebot_msg = await client.send_photo(
            chat_id=query.message.chat.id,
        ...
        await query.message.edit_text(
            text=script.MOVIEBOT_DONATION.format(query.from_user.mention, QR_CODE, OWNER_UPI_ID),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
        ...
        buttons = [[
            InlineKeyboardButton('𝓜𝓸𝓿𝓲𝓮𝓫𝓸𝓽 📜', url=''),
            InlineKeyboardButton('⇋ ʙᴀᴄᴋ ⇋', callback_data='about')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
