...
from moviebot.util.file_properties import get_name
...
async def extract_data_handler(client: Client, query: CallbackQuery):
    ...
    try:
        ...
        if BIN_CHANNEL == -100:
            await query.message.reply_text("❌ `BIN_CHANNEL` is not configured correctly. Contact Admin.", quote=True)
            return
        ...
        page_parts.append(
            '<i><code>Join <a href="https://t.me/newlinemovies">Moviebot</a></code></i>'
        )
