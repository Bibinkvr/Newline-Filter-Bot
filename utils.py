...
from plugins.Moviebotfutures.Imdbposter import get_movie_detailsx
...
async def get_poster(query, bulk=False, id=False, file=None):
    ...
    return {
        ...
        'kind': movie.kind,
        "imdb_id": imdb_id,
        "cast": list_to_str(movie.stars),
        "runtime": list_to_str(movie.duration),
        "countries": list_to_str(movie.countries),
        "certificates": list_to_str(movie.certificates),
        "languages": list_to_str(movie.languages),
        "director": list_to_str(movie.directors),
        "writer": list_to_str([p.name for p in movie.writers]),
        "producer": list_to_str([p.name for p in movie.producers]),
        "composer": list_to_str([p.name for p in movie.composers]),
        "cinematographer": list_to_str([p.name for p in movie.cinematographers]),
        "music_team": list_to_str([p.name for p in movie.music_team]),
        "distributors": list_to_str([c.name for c in movie.distributors]),        
        'release_date': date,
        'year': movie.year,
...
# list_to_str consolidated above
                ...
                if imdb:
                    ...
                    if ULTRA_FAST_MODE:
                        cap = (
                            ...
                            f"⚜️ ᴘᴏᴡᴇʀᴇᴅ ʙʏ :⚡ {query.message.chat.title or temp.B_LINK or 'ᴍᴏᴠɪᴇʙᴏᴛ'}\n</b>"
                        )
                    else:
                        cap = (
                            f"<b>🏷 ᴛɪᴛʟᴇ : <code>{search}</code>\n"
