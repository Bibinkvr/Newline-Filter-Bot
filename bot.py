...
def moviebot_plugins_handler(app, plugins_dir: str | Path = "plugins", package_name: str = "plugins") -> list[str]:
...
async def moviebot_start():
    print('\n\nInitalizing Moviebot')
    await moviebot.start()
    bot_info = await moviebot.get_me()
    moviebot.username = bot_info.username
    await initialize_clients()
    loaded_plugins = moviebot_plugins_handler(moviebot)
    if loaded_plugins:
    ...
    if LOG_CHANNEL and LOG_CHANNEL != -100:
        try:
            await moviebot.send_message(chat_id=LOG_CHANNEL, text=script.RESTART_TXT.format(temp.B_LINK, today, time))
        except Exception as e:
    ...
    moviebot.loop.create_task(keep_alive())
    await idle()
...
if __name__ == '__main__':
    ...
    while True:
        try:
            loop.run_until_complete(moviebot_start())
            break  
